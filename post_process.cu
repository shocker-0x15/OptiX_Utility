...
extern "C" __global__ void postProcess(const float4* rawOutputBuffer, uint32_t imageSizeX, uint32_t imageSizeY, uint32_t numAccumFrames,
                                       float4* outputBuffer) {
    ...
    float3 pix = getXYZ(rawOutputBuffer[idx]) / (float)numAccumFrames;
